<template>
    <div class="app-container">
        <div class="filter-container">
            <el-button class="addBtn" type="primary" @click="dialogVisible = true,dialogType='add'">添加帮助</el-button>
        </div>
        <el-table
        ...
        <el-dialog :title="dialogType === 'edit' ? '编辑' : '添加'" :visible.sync="dialogVisible">
            <el-form>
                <el-form-item label="问题">
                    <el-input v-model="details.wenti" placeholder="请输入问题" />
                </el-form-item>
                
                <el-form-item label="答案">
                    <el-input type="textarea" v-model="details.answer" placeholder="请输入答案" />
                </el-form-item>
              </el-form>
...
import { createhelp,gethelplist,changehelp,delhelp } from "@/api/article"
export default {
    name: "ComplexTable",
    ...
    methods: {
        del(scope){
            this.$confirm('是否确认删除该问题?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                this.listLoading = true;
                delhelp(scope.row.id).then((response) => {
        ...
        toWait(){
            ...
            if(this.dialogType == 'edit'){
                changehelp(this.details).then((response) => {
                    console.log(response);
                    if (response.errcode == 0) {
                ...
                createhelp(this.details).then((response) => {
                    console.log(response);
                    if (response.errcode == 0) {
                        self.getlist()
                    } else {
        ...
        getlist(){
            ...
            gethelplist(data).then((response) => {
