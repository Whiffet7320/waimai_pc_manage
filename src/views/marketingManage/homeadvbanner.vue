...
<script>
import { gethomebanner, delhomebanner, addhomebanner } from "@/api/set";
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm("您确定删除么?", "警告！", {
        ...
        .then(async () => {
          delhomebanner({ id: row.row.id }).then(res => {
            console.log(res);
            if (res.errcode == 0) {
    ...
    getManageList() {
      ...
      gethomebanner(this.listQuery).then(response => {
