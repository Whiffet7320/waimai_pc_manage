...
import { getCheckList, agreeSchoolCheck } from "@/api/store";
...
export default {
  ...
  methods: {
    ...
    updateData() {
      ...
      if (this.$isEmpty(this.temp.status))
        return this.$message.error("未选择审核类型");
      ...
      agreeSchoolCheck(tempData).then(res => {
        if (res.errcode == 0) {
